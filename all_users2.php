	...
	if( isset($_POST["lettre"])) {
			...
			if(isset($_GET["status_id"], $_GET["user_id"], $_GET["action"])) {
				try {
					...
					$stmt = $pdo->prepare("INSERT INTO action_log (action_date,action_name,user_id) VALUES ('".time()."',':action',':user_id')");
					$stmt->execute(['action' => $_GET['action'],'user_id' => $_GET['user_id']]);
					$pdo->commit();
				}catch (Exception $e){
					$pdo->rollBack();
					throw $e;
